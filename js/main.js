...
function initContact() {
    ...
    if(projectsData && select) {
        projectsData.forEach(p => {
            ...
            if (!select.querySelector(`option[value="${p.title}"]`)) {
    ...
    // 2. HANDLE URL PARAMS (Pre-select project)
    const params = new URLSearchParams(window.location.search);
    const projectSlug = params.get("project");
    const success = params.get("success");

    // Pre-select project logic
    ...
    // 3. SMART REDIRECT (The Fix)
    // This tells the form: "After sending, come back to THIS exact page + ?success=true"
    const redirectInput = document.getElementById("redirect-url");
    if(redirectInput) {
        const currentUrl = window.location.href.split('?')[0]; // Get URL without params
        redirectInput.value = `${currentUrl}?success=true`;
    }

    // 4. SHOW SUCCESS MESSAGE
    // If we just came back from sending the email, hide form & show thank you
    if(success === "true") {
